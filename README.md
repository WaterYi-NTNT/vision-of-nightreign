...
- **[v1.0.3] - 11-07-2026**
  - 新增 `.ini` 配置文件，支持在游戏外手动调整所有参数。
  - 新增开关，支持在进度条旁边显示具体的数值（当前值/最大值）。
  - 新增字体大小调节，不再受限于 UI 整体缩放。
  - 支持在菜单中开启“常驻显示”，即使积蓄为 0 也会显示空的进度条。
  - 现在可以使用 `~` 或 `Insert` 键呼出设置菜单。
- **[v1.0.2] - 06-05-2026**
...
- **[v1.0.4] - 07-15-2026**
  - 将原本的外部叠加层重构为游戏内 Hook 渲染，不再依赖独立覆盖窗口。
    - 这修复了外部 Overlay 方案在全屏、焦点切换和显示层级下容易出现的显示问题。
    - 现在 UI 会直接绘制在游戏画面中，整体显示效果和稳定性更好。
  - 新增一个默认 ini 配置文件